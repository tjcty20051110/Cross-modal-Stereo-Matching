...
**右下：Siamese ResNet18 + SGM** — 相比 Census 略有改善，能勉强分辨出路面与天空的粗略分界，但树木轮廓断裂、车辆边界空洞严重，远景区域布满噪声斑块。该方案仅在小块局部区域降低了代价随机性，未能解决跨模态匹配的本质难题，最终因边缘断裂、语义混淆与泛化性差被团队放弃（详见 4.4 节）。此对比充分说明：跨模态立体匹配的核心瓶颈在于**特征表示**，但单纯的局部嵌入 + 传统聚合远不足以达到可用标准。
...
### 8.1 CFM Pipeline 综合六宫格对比（可视化改进后）
