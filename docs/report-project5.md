...
> **摘要**：本实验围绕 RGB 与近红外（NIR/IR）跨模态立体匹配问题，在 MS2 多光谱立体数据集上对比验证了三种方案：传统 Census + SGM 基线、基于 Siamese ResNet18 的学习型特征替换 Census 后再接入 SGM 的混合方案，以及基于 arXiv:2411.03638 的 CFM 端到端深度学习流水线。实验结果表明，传统方法在跨模态场景下几乎完全失效（EPE ≈ 55 px，D1-all ≈ 95%），而 Siamese ResNet18 + SGM 方案将 EPE 降至 10.12 px，验证了学习型模态不变特征的有效性。CFM Pipeline 虽在 50 epoch 内实现损失下降 94%，但受限于训练数据量与代价体分辨率，验证精度尚不及 Siamese 方案。本文以样本 **008004_4** 为典型案例，从训练损失、定量指标与定性可视化三个维度展开详细分析，并深入探讨不同方法表现差异的成因。
...
- **未饱和趋势**：第 15  epoch 曲线仍呈下行态势，延长训练至 50 epoch 预计可进一步降低；
- **跨模态代价**：与同模态训练（15 epoch 可降至 ~0.006）相比，跨模态 loss 绝对值更高，这正反映了 NIR 与 RGB 之间存在真实的模态差距，网络需要更强的判别力才能将匹配对与不匹配对拉开。
...
|------|:----------:|:---------:|:--------:|:----------:|----------|
| Census + SGM（传统） | 10 | 4529.04 | 55.56 | 95.03 | — |
| **Siamese ResNet18 + SGM** | 10 | **732.43** | **10.12** | **27.61** | `models/siamese_cross.pt` |
| CFM Pipeline | 20 | 3591.49 | 38.02 | 97.07 | `models/cfm.pt` |
...
上图展示了 Siamese ResNet18 + SGM 在样本 008004_4 上的完整推理结果。从左至右、从上至下依次为：

- **Left RGB**：参考图像（实际为 NIR 左图经伪彩色映射后的可视化，此处标签为 RGB 表示参考视角）；
- **Right IR**：匹配图像（RGB 右图）；
- **Prediction overlay**：预测视差以 Jet 伪彩色叠加在左图之上，可直观观察深度与场景结构的对应关系；
- **Predicted disparity**：全分辨率预测视差图，经 Robust percentile stretch 增强对比度。近景（路面下方）呈暖色（高视差），远景（天空与远处树木）呈冷色（低视差），层次分明；
- **Ground truth**：稀疏 LiDAR 投影的真值视差点，为验证评价提供基准；
- **Absolute error**：仅在有 GT 的像素上计算绝对误差。大部分有效像素误差低于 6 px，仅少数远处与遮挡边界出现黄色高亮（误差接近 12 px）。
...
### 8.3 预测视差（Siamese ResNet18 + SGM）

![008004_4 预测视差](report_project5_assets/008004_4_pred.png)

Siamese 方案输出的视差图为**稠密预测**，所有像素均赋予一个视差值（后处理中已通过空洞填充修复无效区域）。与 GT 对比可发现：
- **路面**：从近到远呈现平滑的视差梯度，符合透视几何；
- **左右树木与灌木丛**：轮廓清晰，视差跳变与图像边缘对齐，说明 SGM 的 $P_1/P_2$ 惩罚项有效保留了深度不连续性；
- **车辆与交通标志**：虽 GT 中缺乏对应真值点，但预测视差在物体位置出现局部高视差区域，与语义一致。
...
### 9.1 为什么 Siamese ResNet18 + SGM 表现最佳？

Siamese 方案在当前实验条件下取得 **EPE = 10.12 px** 的显著优势，其成功可归因于以下三点：

| 因素 | 分析 |
|------|------|
| **任务分解合理** | 网络仅需学习“将不同模态的同一像素映射到相近的嵌入向量”这一局部特征问题，而将全局几何一致性、遮挡处理、亚像素精化交由成熟的 SGM 与后处理模块完成。任务目标明确，小数据量下更易收敛。 |
| **训练监督稠密** | Triplet Loss 仅需 GT 视差即可构造三元组，每张 352×1280 图像可采样 256 个三元组，监督信号密集且稳定。相比之下，CFM 直接回归深度，但 MS2 的 GT 深度极为稀疏（有效像素仅占 24%–82%），导致端到端回归的梯度噪声大。 |
| **SGM 全局约束** | 学习型特征提供了有区分度的局部代价，SGM 在此基础上施加 8 方向平滑先验，有效抑制了孤立噪声点。CFM 虽也有 Gaussian 先验，但其单阶段联合训练使各模块相互耦合，小数据下难以同时优化特征匹配、深度分布与退化掩膜。 |
...
因此，跨模态立体匹配问题的本质瓶颈在于**特征表示**，而非**聚合策略**。Siamese CNN 通过数据驱动学习到的 32 维嵌入，成功绕过了手工特征对灰度一致性的强假设，是其性能跃升的根本原因。
...
| ✅ Siamese ResNet18 + SGM 是当前最佳方案 | EPE 10.12 px，D1-all 27.61%，较传统方法提升 81.8%，证明学习型模态不变特征可有效跨越 RGB↔NIR 模态鸿沟。 |
| ⚠️ CFM Pipeline 训练充分但验证精度受限 | 50 epoch 损失下降 94%，但受数据量、分辨率与稀疏监督制约，EPE 38.02 px 未达预期。 |
| ✅ 任务分解优于端到端（小数据场景） | Siamese 将“特征学习”与“几何推理”解耦，小数据下更易泛化；CFM 的端到端联合优化在数据充足时潜力更大。 |

### 10.2 未来改进方向

1. **延长 Siamese 训练**：当前仅 15 epoch，loss 仍在下降。扩展至 50 epoch 预计可进一步降低 EPE 至 8 px 以下。
