...
- **训练-推理鸿沟**：尽管训练损失持续下降，但如 5.2 节所述，实际推理效果远未达预期。Triplet Loss 仅约束了稀疏 GT 采样点上的局部嵌入距离，对全图稠密推理缺乏有效监督，导致训练指标与视觉质量严重脱节；
...
在完成传统 Census + SGM 与 Siamese ResNet18 + SGM 的实现后，本节将两者的跨模态匹配效果进行统一对比，并阐述最终放弃 ResNet 路线的原因。

### 5.1 传统 Census + SGM 跨模态实验效果及放弃原因
...
**右下：Siamese ResNet18 + SGM** — 相比 Census 略有改善，能勉强分辨出路面与天空的粗略分界，但树木轮廓断裂、车辆边界空洞严重，远景区域布满噪声斑块。该方案仅在小块局部区域降低了代价随机性，未能解决跨模态匹配的本质难题，最终因边缘断裂、语义混淆与泛化性差被团队放弃（详见 5.2 节）。此对比充分说明：跨模态立体匹配的核心瓶颈在于**特征表示**，但单纯的局部嵌入 + 传统聚合远不足以达到可用标准。
...
### 5.2 Siamese ResNet18 + SGM 推理效果与放弃原因

为验证 Siamese ResNet18 在真实跨模态场景中的匹配能力，我们将训练得到的特征提取器接入 SGM 流水线，对验证集样本进行推理。综合可视化结果与定量分析，团队认为 Siamese ResNet18 + SGM 方案在实际跨模态场景中的**可用性极差**，具体问题如下：
...
**训练效果综合分析**：从损失下降曲线可以看出，CFM Pipeline 在 50 个 epoch 内实现了高达 **94.4%** 的 loss 降幅（189.5 → 10.6），说明网络整体优化方向正确，各子模块（CFM 代价体、MDP 分布估计、Degradation Masking 与 Depth 回归）能够协同收敛。然而，损失曲线进入平台期后下降极其缓慢，验证集 EPE 仍高达 38 px 左右，这表明：
1. **数据量瓶颈**：200 张训练图像不足以支撑 360k 参数网络的充分收敛，模型处于欠拟合状态；
2. **分辨率瓶颈**：1/4 分辨率代价体导致高频细节丢失，仅靠上采样无法恢复精细边缘；
3. **稀疏监督瓶颈**：GT 有效像素仅占全图 24%–82%，大量区域缺乏梯度引导，网络倾向于输出平滑插值。
因此，后续应优先从扩充数据、提升代价体分辨率以及引入稠密监督三个方向进行改进。

### 6.4 可视化方案与效果展示

为了更直观地评估 CFM Pipeline 的推理质量，本项目对最终可视化流程进行了系统性改进。原始可视化采用简单的 2×2 网格拼接，存在文字标签在明亮区域难以辨认、稀疏 LiDAR 真值几乎不可见、误差热力图被噪声主导、视差归一化受离群值影响导致有用区域颜色坍缩等问题。改进后的可视化方案使最终输出成为可直接用于报告与演示的实验结果图，具体改进如下。

**鲁棒的视差颜色缩放**：视差渲染器不再直接使用全局最大值进行归一化，而是收集所有有效视差像素后，使用高分位数作为实际显示范围，并与 `config.max_disparity` 钳制，最终应用 `COLORMAP_TURBO` 进行伪彩色映射。即使存在少数离群像素，局部视差细节仍然清晰可见，颜色范围得到高效利用。

**稀疏真值增强**：MS2 数据集的真值视差来自 LiDAR 投影，具有显著的稀疏性。直接渲染时，有效点仅为黑色背景中的微小彩点，几乎不可辨识。改进方案根据有效视差构建掩码，估计稀疏程度后，使用小型椭圆形核对有效掩码和着色视差图同步进行膨胀操作。该方法在不修改底层数值数据的前提下，使道路扫描线、边界和深度层次的视觉结构清晰可辨。

**改进的误差热力图**：误差图不再在全图范围内计算，而是仅在有效真值像素上显示绝对视差误差，并采用鲁棒分位数缩放与 `COLORMAP_INFERNO` 色阶。对于稀疏掩码，同样进行适度膨胀以提升可见性。改进后的误差图能够精确指示预测与真值偏离的位置，避免了无效区域对视觉的干扰。

**仪表盘式对比图与预测叠加**：最终的 `*_comparison.png` 从 2×2 网格重新设计为结构化的 2×3 仪表盘布局，包含：

| 面板 | 内容 | 说明 |
|------|------|------|
| 1 | Left RGB | 参考图像（NIR 左图） |
| 2 | Right IR | 匹配图像（RGB 右图） |
| 3 | Prediction overlay | 预测视差伪彩色叠加在左图上，便于对齐判断 |
| 4 | Predicted disparity | 经鲁棒缩放的 CFM 预测视差图 |
| 5 | Ground truth | 经膨胀增强的稀疏 LiDAR 真值 |
| 6 | Absolute error | 仅在有效 GT 上的绝对误差热力图 |

每个面板配有深色标题栏、稳定宽高比的内容区域、底部副标题以及带像素单位的颜色条，整体边距、背景与边框保持一致。新增的面板 3（预测叠加图）通过将伪彩色视差图混合到左侧 RGB 图像上，使道路边缘、树木和车辆等场景结构的深度对齐情况一目了然。

**收益总结**：上述可视化改进并未修改任何预测或评估数据，仅优化了展示方式，但显著提升了 CFM Pipeline 推理结果的可读性与可解释性。鲁棒的缩放避免了离群值主导，稀疏增强使真值结构清晰可见，仪表盘式布局实现了预测、真值与误差的直观对比，为后续定性分析与报告撰写提供了高质量的视觉素材。
...
| 方法 | 验证样本数 | MSE (px²) | EPE (px) | D1-all (%) | 模型文件 |
|------|:----------:|:---------:|:--------:|:----------:|----------|
...
> **注意**：Siamese ResNet18 的 EPE 虽然在扫描线密集区域较低，但因其存在严重的边缘断裂与泛化性问题（见 5.2 节），团队已放弃该路线。CFM Pipeline 是当前重点探索方向。
...
### 7.3 定性分析：典型案例 008004_4

以下以验证集样本 **008004_4** 为例，从 GT、预测视差、绝对误差与综合对比四个维度展开定性分析，单独展示 CFM Pipeline 的训练效果与推理质量。所有图片均来自 `docs/visualizations/` 目录下该样本的可视化结果。

#### 7.3.1 CFM Pipeline 综合六宫格对比（可视化改进后）

以下展示的是 **CFM Pipeline** 在样本 **008004_4** 上的推理结果，可视化效果采用了本文 `docs/visualization.md` 中描述的改进方案（鲁棒颜色缩放、稀疏 GT 增强、仪表盘式布局、预测叠加等）。**注意：`docs/visualizations/` 目录下的所有 008004_4 系列图片均来自 CFM Pipeline 的输出，与 ResNet 无关。**

![008004_4 综合对比（CFM Pipeline）](./visualizations/008004_4_comparison.png)

从左至右、从上至下依次为：

- **Left RGB**：参考图像（NIR 左图）；
- **Right IR**：匹配图像（RGB 右图）；
- **Prediction overlay**：CFM 预测视差以 Jet 伪彩色叠加在左图之上，可直观判断深度与场景结构的对齐程度；
- **Predicted disparity**：CFM 全分辨率预测视差图，经 Robust percentile stretch 增强对比度。近景路面呈暖色（高视差），远景天空呈冷色（低视差），整体深度趋势正确；
- **Ground truth**：稀疏 LiDAR 投影的真值视差点；
- **Absolute error**：仅在有效 GT 像素上计算的绝对误差。大部分路面区域误差较低，但远处树木边缘与遮挡边界存在黄色高亮（误差接近 12 px）。
...
#### 7.3.3 CFM Pipeline 预测视差

![008004_4 CFM 预测视差](./visualizations/008004_4_pred.png)

CFM Pipeline 输出的视差图为**稠密预测**（由深度图经 $disp = f \cdot b / depth$ 转换得到）。与 GT 对比可发现：
- **路面**：从近到远呈现平滑的视差梯度，符合透视几何；
- **左右树木与灌木丛**：轮廓基本可辨，视差跳变与图像边缘大致对齐，但边界过渡不如传统 SGM 锐利（受限于 1/4 分辨率代价体 + 双线性上采样）；
- **车辆与交通标志**：预测视差在物体位置出现局部高视差区域，与语义一致，但精细结构（如车灯、标志牌）因低分辨率而模糊；
- **远景天空区域**：整体趋势正确，但存在轻微的纹理噪声，这是低分辨率代价体在高频区域的固有局限。

#### 7.3.4 CFM Pipeline 绝对误差分布

![008004_4 CFM 绝对误差](./visualizations/008004_4_error.png)

误差图以 `inferno` 色阶展示（紫→黄表示 0 → 12 px）：
- **低误差主体**：约 65% 有效 GT 像素的误差低于 3 px（深紫色），集中在路面中央连续区域；
- **中等误差带**：部分路面扫描线终点与树木边缘呈现粉红色（3–6 px），主要源于 CFM 1/4 分辨率代价体在精细边缘处的定位偏差；
- **高误差 outliers**：少数黄色亮点位于远处车辆边缘与遮挡边界，这些区域在 CFM 的 Degradation Masking 中被标记为不确定性较高，但 Depth Module 仍输出了与真值存在偏差的深度估计。
...
2. **Siamese ResNet18 + SGM**：视差图存在大面积黑色空洞（SGM 左右一致性检查失败的区域），边缘断裂严重，远景区域被大量噪声斑块覆盖。虽然部分路面扫描线附近的 EPE 看似较低，但全图稠密推理质量极差，存在严重的语义混淆与结构失真，**实际可用性不足，已被团队放弃**。

3. **CFM Pipeline**：整体深度趋势正确（近处暖色、远处冷色），输出稠密且无大面积空洞。虽因 1/4 分辨率代价体 + 双线性上采样导致像素级精细结构（如树木边缘、道路标线）略显模糊，但相比 ResNet 的断裂与空洞，CFM 的连续性与一致性更优，是当前重点优化的方向。

---

## 八、结果成因分析

### 8.1 为什么 Siamese ResNet18 + SGM 最终被放弃？
...
| ❌ Siamese ResNet18 + SGM 训练损失下降，但推理效果不理想 | EPE 指标在稀疏 GT 下具有欺骗性；实际推理存在边缘断裂、语义混淆、SGM 参数敏感等问题，团队已放弃该路线（详见 5.2 节）。 |
